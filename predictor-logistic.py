...
# Train model for dodatkové číslo
...
# Select the most likely dodatkové číslo
predicted_bonus_number = np.argsort(y_bonus_pred_probs[0])[-1] + 1  # Single best dodatkové číslo

# Load latest actual results
last_draws = df.iloc[:ACCURACY_CHECK, 4:10].values.flatten()  # Last main numbers
last_bonus = df.iloc[:ACCURACY_CHECK, [10, 17]].values.flatten()  # Last dodatkové číslo

# Count how many of the predicted numbers appeared in the last 10 draws
correct_main = len(set(predicted_numbers) & set(last_draws))
correct_bonus = len(set([predicted_bonus_number]) & set(last_bonus))

# Display results
print("-" * 50)
print("\n🎟️ Predicted Best Numbers for Next Draw:", sorted(predicted_numbers))
